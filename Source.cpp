...
Eigen::Vector3d vectorToUReuler(pcl::PointXYZ point)
{
	Eigen::Vector3d dirvec;
	Eigen::Vector3d rpy;
	Eigen::Vector3d rotateVec;
	double roll, pitch = 0, yaw = 0;
	{
		dirvec(0) = point.x;
		dirvec(1) = point.y;
...
Eigen::Vector3d vectorToURRPY(pcl::PointXYZ point)
{
	Eigen::Vector3d rpy;
...
#pragma region Variable
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_Z0(new pcl::PointCloud<pcl::PointXYZ>);
...
#pragma region Create Normal vector and match with Edge points
	...
	//	rotateVec = vectorToUReuler(point);


	//	fprintf(outfp, "%.3f, %.3f, %.3f, %.3f, %.3f, %.3f\n", cloudNormal->points[i].x, cloudNormal->points[i].y, cloudNormal->points[i].z, rotateVec(0), rotateVec(1), rotateVec(2));
	//}
	//
	//
	//fclose(outfp);
...
//reader.read("D:\\JasonWork\\PCD\\block_leaf0508Edge.pcd", *cloud);
	...
	for (size_t i = 0; i < tmpcloudNormalA->size(); i++)
	{
		normal.x = tmpcloudNormalA->points[i].normal_x;
		normal.y = tmpcloudNormalA->points[i].normal_y;
		normal.z = tmpcloudNormalA->points[i].normal_z;

		//tmpcloudNormalA->points[i].x += (-35.4);
		//tmpcloudNormalA->points[i].y += (-719.5);
		//tmpcloudNormalA->points[i].z += (-574.3);
...
#pragma region Viewer
	...
	viewer->addPointCloudNormals<pcl::PointXYZINormal>(tmpcloudNormalA, 1, -50, "cloudNormal");
	viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 1.0, 0.0, 1.0, "cloudNormal");
